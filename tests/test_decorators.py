import pytest
from toolops import tool, readonly, sideeffect, stateful, cache_manager
...
async def test_tool_decorator_initialization_variants():
    ...
    async def t1(): return 1
    
    @readonly(cache_backend="memory")
    async def t2(): return 2
    
    @sideeffect()
    async def t3(): return 3
    
    @stateful(cache_backend="memory")
    async def t4(): return 4
